...
void ntt(int16_t r[NTRUOAEP_N], const int16_t a[NTRUOAEP_N])
{
	...
	{
		t1 = fqmul(zeta1, a[i + NTRUOAEP_N/2]); // = \ksi_6 * lower half
	...
	for(int step = NTRUOAEP_N/6; step >= 12; step = step/3)
	{
		...
		{
			zeta1 = zetas[k++];
			zeta2 = zetas[k++]; //R zeta2 = R zeta1^2

			for(int i = start; i < start + step; i++)
			{
				t1 = fqmul(zeta1, r[i +   step]); // xi1 = zeta1/R*r[i +   step]
				t2 = fqmul(zeta2, r[i + 2*step]); // xi2 = zeta2/R
				t3 = fqmul(-714, t1 - t2); 		  // omega3(xi1-xi2)
				...
				if (step == 12)
				{
					r[i + 2*step] = barrett_reduce(r[i] - t1 - t3); // With |t1|,|t2|,|t3| <= 4000, it will be safe to do barret reduce only at step==12;
...
void invntt(int16_t r[NTRUOAEP_N], const int16_t a[NTRUOAEP_N])
{
	...
	for(int step = 3; step <= 6; step <<= 1)
	{
		for(int start = 0; start < NTRUOAEP_N; start += (step << 1))
		{
			...
			for(int i = start; i < start + step; i++)
			{
				...
				r[i + step] = fqmul(zeta1,  t1 - r[i]);  // zeta1 * (-2 a[i])
				if(step==3)
				{
					r[i       ] = r[i] + t1; //barrett_reduce(r[i] + t1); // 2 a[i]
				}
				else
				{
					r[i       ] = barrett_reduce(r[i] + t1);
	...
	for(int step = 12; step <= NTRUOAEP_N/6; step = 3*step)
	{
		for(int start = 0; start < NTRUOAEP_N; start += 3*step)
		{
			...
			for(int i = start; i < start + step; i++)
			{
				...
				t2 = fqmul(zeta1, r[i + 2*step] - r[i]        + t1); // -2t1 - t3 -t2  + omega3(-2t2+t3-t1) = -(2+omega3)t1+(omega3-1)(omega3)(t1-t2) -(2omega3+1)t2
	...
	for(int i = 0; i < NTRUOAEP_N/2; i++)
	{
		t1 = r[i] + r[i + NTRUOAEP_N/2]; // = 2a[i] + a[i+N/2]
		t2 = fqmul(2394, r[i] - r[i + NTRUOAEP_N/2]); // (2\kxi6-1)* a[i+N/2]  (-1665 = (2 * -722(kxi6) -1)^{-1}* 2^16 mod 3457)
...
*              MODIFIED FROM THE OLD VERSION IN Zq, q=3457
*              NOT IMPLEMENTED
*
* Arguments:   - int16_t a: first factor a = x mod q
*
* Returns 16-bit integer congruent to x^{-1} * R^2 mod q
* Currently Deprecated; not modified for new Q
**************************************************/
static int16_t fqinv(int16_t a)
{
	//target : 11011 11010 111
	int t1, t2, t3, t4;

	t1 = fqmul(a,   a); //10   R=V-1;  
	t2 = fqmul(t1,  a); //11
	t3 = fqmul(t2, t2); //110
	t3 = fqmul(t3, t3); //1100
	t3 = fqmul(t3, t3); //11000
	t1 = fqmul(t3, t1); //11010

	t3 = fqmul(t3, t2); //11011
	t4 = fqmul(t3, t3); //110110
	t4 = fqmul(t4, t4); //1101100
	t4 = fqmul(t4, t4); //11011000
	t4 = fqmul(t4, t4); //110110000
	t4 = fqmul(t4, t4); //1101100000
	t4 = fqmul(t4, t1); //1101111010
	
	t4 = fqmul(t4, t4); //11011110100
	t4 = fqmul(t4, t4); //110111101000
	t4 = fqmul(t4, t2); //110111101011
	t4 = fqmul(t4, t4); //1101111010110
	t4 = fqmul(t4, a);  //1101111010111
...
*              MODIFIED FROM THE OLD VERSION IN Zq[X]/(X^4-zeta)
*              NOT IMPLEMENTED
*
...
int  baseinv(int16_t r[3], const int16_t a[3], int16_t zeta)
{
	...
	t0 = montgomery_reduce(a[1]*a[1]-3*a[0]*a[2]); // a1^2 - 3a0a2
	t4 = montgomery_reduce(a[2]*zeta); // a2zeta
