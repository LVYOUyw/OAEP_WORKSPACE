...
void poly_frombytes(poly *r, const uint8_t a[NTRUOAEP_POLYBYTES])
{
	...
	for(int i = 0; i < 81; i++)
	{
		for (int j = 0; j < 13; j++) t[j] = a[13*i + j];

		r->coeffs[8*i    ]  =  (t[0])        |   ((int16_t)t[1] & 0x1f) << 8; 
		r->coeffs[8*i + 1]  =  (t[1] >> 5)   |   ((int16_t)t[2] << 3)            | ((int16_t)t[3] & 0x3) << 11;
		r->coeffs[8*i + 2]  =  (t[3] >> 2)   |   ((int16_t)t[4] & 0x7f) << 6; 				 
		r->coeffs[8*i + 3]  =  (t[4] >> 7)   |   ((int16_t)t[5] << 1)  			 | ((int16_t)t[6] & 0xf) << 9;
		r->coeffs[8*i + 4]  =  (t[6] >> 4)   |   ((int16_t)t[7] << 4)            | ((int16_t)t[8] & 0x1) << 12; 
...
void poly_cbd1(poly *r, const uint8_t buf[NTRUOAEP_N/4])
{
	...
	for(int i = 0; i < 2; i++)
	{
		for(int j = 0; j < 8; j++)
		{
			...
			t2 = load32_littleendian(buf + 32*i + 4*j + 81);
	...
	for(int j = 0; j < 4; j++)
	{
		...
		t2 = load32_littleendian(buf + 4*j + 145);

		for (int k = 0; k < 2; k++)
		{
			for(int l = 0; l < 16; l++)
			{
				r->coeffs[512 + 8*l + 2*j + k] = (t1 & 0x1) - (t2 & 0x1);
...
int poly_cbd1_inv(uint8_t *w, const poly *a, const uint8_t buf[NTRUOAEP_N/8]) {
	...
	for(int j = 0; j < 4; j++)
	{
		t2 = load32_littleendian(buf + 4*j + 64);
		t3 = 0;
...
int poly_sotp_inv(uint8_t *msg, const poly *a, const uint8_t *buf)
{
	...
	{
		t1 = load32_littleendian(buf + 4*j + 64);
		t2 = load32_littleendian(buf + 4*j + 64 + NTRUOAEP_N/8);
		t3 = 0;
...
void poly_basemul(poly *r, const poly *a, const poly *b)
{
	for(int i = 0; i < NTRUOAEP_N/6; ++i)
	{
		basemul(r->coeffs + 6*i, a->coeffs + 6*i, b->coeffs + 6*i, zetas[108 + i]);
		basemul(r->coeffs + 6*i + 3, a->coeffs + 6*i + 3, b->coeffs + 6*i + 3, -zetas[108 + i]);
	}
}
...
void poly_basemul_add(poly *r, const poly *a, const poly *b, const poly *c)
{
	for(int i = 0; i < NTRUOAEP_N/6; ++i)
	{
		basemul_add(r->coeffs + 6*i, a->coeffs + 6*i, b->coeffs + 6*i, c->coeffs + 6*i, zetas[108 + i]);
		basemul_add(r->coeffs + 6*i + 3, a->coeffs + 6*i + 3, b->coeffs + 6*i + 3, c->coeffs + 6*i + 3, -zetas[108 + i]);
	}
}
...
*
* Description: pack polynomial with given randomness bytes
*
...
void poly_oaep(poly *s, poly *t, const uint8_t *r, const uint8_t *m) 
{
	...
	//shake256(buff, NTRUOAEP_N / 4, r, NTRUOAEP_N / 4);
	hash_g(buff, t_byte);
...
void poly_tomontgomery(poly *r, const poly *a)
{
	for (int i = 0; i < NTRUOAEP_N; i++)
	{
		r->coeffs[i] = montgomery_reduce(a->coeffs[i]*1440);
	}
}
